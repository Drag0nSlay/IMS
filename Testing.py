...
from tkinterweb import HtmlFrame  # Import HtmlFrame for web content
...
class IMS:
    def __init__(self, root):
        ...
        btn_powerbi=Button(LeftMenu,text="Power BI Report",command=self.open_powerbi_report,image=self.icon_side,compound=LEFT,padx=5,anchor="w",font=("times new roman",14,"bold"),bg="white",bd=3,cursor="hand2")
        btn_powerbi.pack(side=TOP,fill=X)
...
#------------------------------------------------------------------------------------------------------
    ...
    def open_powerbi_report(self):
        self.new_win = Toplevel(self.root)
        self.new_win.title("Power BI Report")
        self.new_win.geometry("800x600")
        
        # Create a web view
        frame = HtmlFrame(self.new_win)
        frame.pack(fill="both", expand=True)
        
        # Load Power BI report
        powerbi_url = "YOUR_POWERBI_REPORT_EMBED_URL"
        frame.load_website(powerbi_url)
